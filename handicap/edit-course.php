...
$ajax['location_id'] = JFilterInput::clean($ajax['location_id'],'int');
$ajax['country_id'] = JFilterInput::clean($ajax['country_id'],'int');

if ($ajax['save']) {
  $notice = $this->edit_course($ajax);
}
else {
  $notice = $this->course_from_user($user->id,$ajax);
}

//$countries = $this->list_countries();
//$locations = $this->list_locations();
$locations = $locations = $this->list_places();

$ajax['country'] = $locations[$ajax['country_id']]['name'];
$ajax['location'] = $locations[$ajax['location_id']]['name'];
...
<?php } ?>
<form id="hc-edit-course" action="" method="" onsubmit="javascript:return Handicap.submit();">
  <table style="width:100%;">
    ...
    <tr>
      <td colspan="2">
        <br />
        <input type="hidden" name="ajax[course_id]" value="<?php echo number_format($ajax['course_id'],0) ?>" />
